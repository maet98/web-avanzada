...
public class FuncionesDynamoDbReservation {
    ...
    public ListarReservationResponse listarReservations(FiltroListaReservation filtro, Context context, Boolean pasado) {
        ...
        do {// La consulta vía ScanRequest solo retorna 1 MB de datos por iteracion,
            ...
            for (Map<String, AttributeValue> mapReservation : rows) {
                ...
                //
                AttributeValue reservaIdAtributo = mapReservation.get("reservationId");
                ...
                AttributeValue cantidad = mapReservation.get("cantidad");
                //
                ...
                if(cantidad!=null){
                    tmp.setCantidad(Integer.valueOf(cantidad.getN()));
                }
                if(fechaReserva != null) {
                    tmp.setFechaReserva(fechaReserva.getS());
                }
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'")
                        .withZone(ZoneId.of("UTC"));
                LocalDateTime date = LocalDateTime.parse(tmp.getFechaReserva(), formatter);
                if(pasado && now.isAfter(date)){
                    reservations.add(tmp);
                } else if( !pasado && date.isAfter(now)){
                    reservations.add(tmp);
                }
            }
    ...
    public ReservationResponse eliminarReservation(Reservation reservation, Context context){
        ...
        DeleteItemOutcome outcome = table.deleteItem("reservationId", reservation.getReservationId());
        return new ReservationResponse(false, null, reservation);
    }
